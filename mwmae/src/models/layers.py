...
class Attention(nn.Module):
    ...
    def __call__(self, x, train: bool = True):
        ...
        scale = head_dim ** -0.5
        qkv_layer = nn.Dense(self.dim * 3, dtype=self.dtype, param_dtype=self.dtype, use_bias=self.qkv_bias, kernel_init=dense_kernel_init)
        proj_layer = nn.Dense(self.dim, dtype=self.dtype, param_dtype=self.dtype, kernel_init=dense_kernel_init)
...
def get_relative_position_index1d(win_w, dtype=jnp.float32):
    ...
    # coords = torch.stack(torch.meshgrid([torch.arange(win_w)]))  # 1, Ww
    coords_flatten = jnp.stack(jnp.meshgrid(jnp.arange(win_w)))
    # print("coords shape:", coords_flatten.shape)
...
class WindowedAttentionHead(nn.Module):
    ...
    def setup(self):
        ...
        if self.shift_windows:
            # print("USING SHIFTING WINDOWS..")
            self.shift_size = self.window_size // 2
        else:
            # print("NOT USING SHIFTING WINDOWS..")
            self.shift_size = 0
    ...
    def _get_rel_pos_bias(self):
        ...
        relative_position_bias = self.relative_position_bias_table[
            relative_position_index.reshape(-1)].reshape(self.window_area, self.window_area, -1)  # Wh*Ww,Wh*Ww,nH
...
class WindowedMultiHeadAttention(nn.Module):
    ...
    def setup(self):
        ...
        self.qkv = nn.Dense(self.dim * 3, dtype=self.dtype, param_dtype=self.dtype, use_bias=self.qkv_bias, kernel_init=dense_kernel_init)
        ...
        self.proj = nn.Dense(self.dim, dtype=self.dtype, param_dtype=self.dtype, kernel_init=dense_kernel_init)
        if self.proj_drop != 0:
            self.drop_layer = nn.Dropout(rate=self.attn_drop, name="proj_drop_layer")
            # self.drop_layer = partial(nn.Dropout, rate=self.proj_drop, name="proj_drop_layer")
        else:
            self.drop_layer = None
    ...
    def __call__(self, x, train: bool = True):
        ...
        o = []
        for i in range(self.num_heads):
            head_i = self.attn_heads[i](q[i], k[i], v[i])[jnp.newaxis, Ellipsis]
            o.append(head_i)
...
class Block(nn.Module):
    ...
    def __call__(self, x, train: bool = True):
        ...
        outputs1 = self.norm_layer(dtype=self.dtype, param_dtype=self.dtype)(x)
        ...
        outputs2 = self.norm_layer(dtype=self.dtype, param_dtype=self.dtype)(x)
